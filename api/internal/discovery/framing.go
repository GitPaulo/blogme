...
//
// Two headers say it. X-Frame-Options speaks only of DENY and SAMEORIGIN, both of
// which exclude us. Content-Security-Policy's frame-ancestors replaces it and takes a
// source list, and anything short of a wildcard is read here as a refusal: the only
// list that would let the app through is one that names it, and no blog names it.
// Being wrong that way costs a preview we decline to open; being wrong the other way
// costs a frame the browser blocks and a console error.
//
...
// framableByAnyone reports whether a frame-ancestors source list lets an arbitrary site
// frame the page. Only a wildcard and a bare https: scheme do; 'none', 'self' and a
// list of hosts all describe somewhere this app is not. An empty list is 'none' by
// omission.
