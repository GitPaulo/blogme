<script lang="ts">
	...
	import Drawerhead from 'flowbite-svelte/Drawerhead.svelte';
	import Heading from 'flowbite-svelte/Heading.svelte';
	import Input from 'flowbite-svelte/Input.svelte';
	import Modal from 'flowbite-svelte/Modal.svelte';
	import P from 'flowbite-svelte/P.svelte';
	import Tooltip from 'flowbite-svelte/Tooltip.svelte';
	import VirtualList from 'flowbite-svelte/VirtualList.svelte';
	...
	/**
	 * Everything behind the bookmarks button: the panel itself, the two dialogs it opens,
	 * and the reading, filtering, importing and exporting that only exist while it is up.
	 *
	 * Split from the button that opens it so that none of this — the drawer, the modals, the
	 * virtual list, the file reader — is in the chunk a reader downloads before they have
	 * asked for any of it. The button knows how to fetch this; see BookmarksPanel.svelte.
	 *
	 * The count on the button is the one thing that has to be right without this being here,
	 * and it comes from the store, which the button loads itself.
	 */
	...
	// Full records are only read when the drawer is actually opened. This component is not
	// mounted before the first open either, so the first run of this is the first opening.
	$effect(() => {
		if (!open) return;
